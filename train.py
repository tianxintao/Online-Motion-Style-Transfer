...
class ArgParserTrain(argparse.ArgumentParser):
    def __init__(self, *args, **kwargs):
        ...
        self.add_argument("--dis_lr", default=1e-4, type=float)
...
class Trainer():
    ...
    def train_discriminator(self, train_data):
        ...
        real_score, grad = self.model.forward_dis(train_data["rotation"], train_data["position"], train_data["velocity"], train_data["input_style"], train_data["content"], compute_grad=True)
    ...
    def train_gen(self, train_data):
        ...
        # reconstruction
        reconstructed_motion = self.model.forward_gen(train_data["rotation"], train_data["position"], train_data["velocity"],
    ...
    def compute_generator_loss(self, reconstruction, fake_motion, ground_truth, fake_score):
        ...
        if fake_motion != None:
            ...
            if self.args.perceptual_loss:
                _, ground_truth_features = self.classification(ground_truth["rotation"], ground_truth["position"], ground_truth["velocity"])
                _, fake_motion_features = self.classification(fake_motion["rotation"], fake_motion["position"], fake_motion["velocity"])
                for ground_truth_key, fake_motion_key in zip(ground_truth_features.keys(), fake_motion_features.keys()):
                    perceptual_loss = perceptual_loss + self.criterion(ground_truth_features[ground_truth_key], fake_motion_features[fake_motion_key])
        else:
        ...
        for epoch in range(self.args.n_epoch):
            for i, train_data in enumerate(self.dataloader):
                ...
                if (i+1) % self.args.log_freq == 0:
                    self.logger.info('Train: Epoch [{}/{}], Step [{}/{}]| g_loss: {:.3f}| d_loss: {:.3f}| gp_loss: {:.3f}| r_loss: {:.3f}| p_loss: {:.3f}| v_loss: {:.3f}| per_loss: {:.3f} | a_loss: {:.3f}' 
            ...
            if (epoch+1) % self.args.test_freq == 0:
                ...
                for i, test_data in enumerate(self.testloader):
                    reconstructed_motion = self.model.forward_gen(test_data["rotation"], test_data["position"], test_data["velocity"],
                    ...
                    if i in j:
                        duration = reconstructed_motion["rotation"].shape[1]
                        ...
                        save_bvh_from_network_output(
                            transferred_motion, 
                            os.path.join(self.video_dir, "{}_{}_{}_to_{}_{}.bvh".format(epoch+1, i, input_style, transferred_style, content))
                        )
            ...
            for i, train_data in enumerate(self.dataloader):
                ...
                if (i+1) % self.args.log_freq == 0:
                    self.logger.info('Train: Epoch [{}/{}], Step [{}/{}]| loss: {:.4f}' 
                        .format(epoch+1, self.args.n_epoch, i+1, len(self.dataloader), mean(self.loss_dict["crossentropy_loss"])))
                    self.reset_loss_dict()
