...
def process_single_bvh(filename, args, downsample=4, skel=None):
    ...
    content_tensor = torch.FloatTensor(np.tile([np.eye(len(content_labels))[content_index]], (episode_length, 1)))
    content_tensor[155:, 0] = 0
    content_tensor[155:, 2] = 1
    data = {
...
def main():
    ...
    transferred_motion = model.forward_gen(test_data["rotation"], test_data["position"], test_data["velocity"],
                                           ...
                                           test_data["input_style"], test_data["transferred_style"], test_time=False)
    ...
    #     transferred_motion, 
    #     os.path.join(args.load_dir, "submission/{}_to_{}_{}.bvh".format(input_style, output_style, content))
    # ) 
    remove_fs(
        transferred_motion,
        foot_contact,
